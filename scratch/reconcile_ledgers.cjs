...
async function run() {
  ...
  for (const comp of companies) {
    ...
    console.log(`Company "${comp.name}": Updated blocked revenue to ₹${blockedSum.toLocaleString("en-IN")}`);
  }

  console.log("All company ledgers reconciled successfully.");
  await mongoose.disconnect();
  process.exit(0);
}

run().catch((err) => {
  console.error("Ledger Reconcile Error:", err);
  process.exit(1);
});
