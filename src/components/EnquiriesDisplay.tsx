...
export default function EnquiriesDisplay() {
  ...
  const admittedEnquiryIdSet = React.useMemo(() => {
    ...
    finalAdmissions.forEach((adm: any) => {
  ...
  const enquiriesForSourceAnalysis = React.useMemo(() => {
    if (sourceFilter) {
      return dateFilteredEnquiries.filter((lead) => {
        if (brandFilter && lead.targetBrand !== brandFilter) return false;
        if (advisorFilter && lead.assignedCrmAdvisor !== advisorFilter) return false;
        if (priorityFilter && lead.priorityLevel !== priorityFilter) return false;
        if (statusFilter && lead.status !== statusFilter) return false;
  ...
  }, [sourceFilter, dateFilteredEnquiries, filteredEnquiries, brandFilter, advisorFilter, priorityFilter, statusFilter]);
  ...
  return (
    <div className="space-y-6 flex-1 flex flex-col justify-between">
              ...
              <>
                <div className="my-auto py-3 flex items-center justify-center">
                  {enquiriesForSourceAnalysis.length > 0 ? (
                    ...
                    >
                      {/* Inner circle with centered count or conversion rate */}
                      {sourceViewMode === "conversion" ? (
      ...
      {/* Client Directory Leads Table (Shared Component across all roles) */}
      <ClientDirectoryLeadsTable
        filteredEnquiries={filteredEnquiries}
        isLoading={isLoading}
        onSelectLead={(lead) => setSelectedLead(lead)}
        onRefresh={fetchEnquiries}
        getAdvisorsForBrand={getAdvisorsForBrand}
