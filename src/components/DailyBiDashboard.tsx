...
export default function DailyBiDashboard() {
  ...
  if (isLoading) {
    return (
      <div className="flex flex-col items-center justify-center py-20 bg-white rounded-3xl border border-slate-200 shadow-sm text-center">
        ...
        <p className="text-sm font-bold text-slate-800">Generating Executive Business Intelligence Report...</p>
        <p className="text-xs text-slate-400 font-medium mt-1">Aggregating revenue, lead conversion funnel, brand analytics & AI insights.</p>
      </div>
    );
  }
  ...
  if (error || !report) {
    return (
      <div className="bg-rose-50 border border-rose-200 rounded-3xl p-8 text-center text-rose-700">
        <p className="text-sm font-bold">Failed to load report</p>
        <p className="text-xs mt-1">{error || "No report data available."}</p>
        <button
          onClick={() => fetchReport(selectedDate)}
          className="mt-4 px-4 py-2 bg-rose-600 text-white rounded-xl text-xs font-bold shadow-xs hover:bg-rose-700"
        >
          Retry Loading
        </button>
      </div>
  ...
  return (
    <div className="space-y-8 font-sans print:space-y-6 print:p-0">
      ...
      {/* HEADER & FILTER BAR */}
      <div className="bg-white border border-slate-200/90 rounded-3xl p-6 shadow-sm flex flex-col md:flex-row items-start md:items-center justify-between gap-4 print:shadow-none print:border-none">
        <div>
          ...
          <h1 className="text-xl sm:text-2xl font-extrabold text-slate-900 tracking-tight mt-1">
            CoachFlow ERP – Enhanced Daily Business Intelligence Report
          </h1>
          <p className="text-xs font-medium text-slate-400 mt-0.5">
            Report Date: <strong className="text-slate-700">{report.dateStr}</strong> • Generated at {report.generatedAtStr}
          </p>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-3 print:hidden">
          ...
          <button
            onClick={() => fetchReport(selectedDate)}
            className="px-3.5 py-2 bg-slate-100 hover:bg-slate-200 text-slate-700 text-xs font-bold rounded-xl transition-all shadow-2xs"
          ...
          <button
            onClick={handlePrint}
            className="px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-extrabold rounded-xl shadow-md shadow-indigo-600/10 transition-all cursor-pointer"
          >
            🖨️ Print / Save PDF
          </button>
        </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { label: "TOTAL REVENUE", val: formatCurrency(executiveSummary.totalRevenue.value), change: executiveSummary.totalRevenue.changePct, color: "text-indigo-600", bg: "bg-indigo-50/50" },
          { label: "TOTAL COLLECTIONS", val: formatCurrency(executiveSummary.totalCollections.value), change: executiveSummary.totalCollections.changePct, color: "text-emerald-600", bg: "bg-emerald-50/50" },
          { label: "TOTAL LEADS", val: executiveSummary.totalLeads.value, change: executiveSummary.totalLeads.changePct, color: "text-blue-600", bg: "bg-blue-50/50" },
          { label: "ADMISSIONS", val: executiveSummary.admissions.value, change: executiveSummary.admissions.changePct, color: "text-purple-600", bg: "bg-purple-50/50" },
          { label: "CONVERSION RATE", val: `${executiveSummary.conversionRate.value}%`, change: executiveSummary.conversionRate.changePct, color: "text-amber-600", bg: "bg-amber-50/50" },
          { label: "OUTSTANDING FEES", val: formatCurrency(executiveSummary.outstandingFees.value), change: executiveSummary.outstandingFees.changePct, color: "text-slate-700", bg: "bg-slate-50" },
          { label: "ESTIMATED LOSS", val: formatCurrency(executiveSummary.businessLoss.value), change: executiveSummary.businessLoss.changePct, color: "text-rose-600", bg: "bg-rose-50/50" },
          { label: "UNCONVERTED LEADS", val: businessLossAnalysis.unconvertedLeads, change: 0, color: "text-violet-600", bg: "bg-violet-50/50" },
        ].map((kpi, idx) => (
          <div key={idx} className={`border border-slate-200/80 rounded-2xl p-4 bg-white shadow-2xs ${kpi.bg}`}>
            <p className="text-[10px] font-extrabold uppercase tracking-wider text-slate-400">{kpi.label}</p>
            <p className={`text-xl font-extrabold mt-1 ${kpi.color}`}>{kpi.val}</p>
      ...
      {/* 2 & 3. DAILY REVENUE TREND (14-30 DAYS) & REVENUE COMPARISON */}
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        
        {/* 14-Day Revenue Trend Line Graph Bar */}
        <div className="lg:col-span-8 bg-white border border-slate-200/90 rounded-3xl p-6 shadow-sm">
          <div className="flex items-center justify-between mb-6">
            <div>
              <h2 className="text-base font-extrabold text-slate-800">Daily Revenue & Collection Trend (14 Days)</h2>
              <p className="text-xs text-slate-400 font-medium">Historical performance to identify growth & decline patterns</p>
            </div>
            ...
            {revenueTrend.map((item, idx) => {
              ...
              return (
                <div key={idx} className="flex-1 flex flex-col items-center gap-1 group relative">
                  {/* Tooltip Hover */}
                  <div className="opacity-0 group-hover:opacity-100 absolute -top-12 bg-slate-900 text-white text-[10px] p-1.5 rounded shadow-lg pointer-events-none transition-opacity z-20 whitespace-nowrap">
                  ...
                  <div className="w-full flex items-end justify-center gap-0.5 h-36">
                    ...
                    <div
                      ...
                      className="w-1/2 bg-emerald-400 hover:bg-emerald-500 rounded-t transition-all"
          ...
          <div className="flex items-center justify-center gap-6 mt-4 text-xs font-bold text-slate-600">
            <div className="flex items-center gap-2"><span className="h-3 w-3 rounded bg-indigo-500"></span> Total Revenue</div>
            <div className="flex items-center gap-2"><span className="h-3 w-3 rounded bg-emerald-400"></span> Total Collections</div>
          </div>
        </div>

        {/* Revenue Comparison (Today vs Yesterday vs Last Week) */}
        <div className="lg:col-span-4 bg-white border border-slate-200/90 rounded-3xl p-6 shadow-sm flex flex-col justify-between">
          <div>
            <h2 className="text-base font-extrabold text-slate-800">Revenue Comparison</h2>
            <p className="text-xs text-slate-400 font-medium mb-6">Today vs Yesterday vs Same Day Last Week</p>
                ...
                return (
                  <div key={idx} className="space-y-1.5">
                    ...
                    <div className="w-full h-3 bg-slate-100 rounded-full overflow-hidden">
                      <div style={{ width: `${widthPct}%` }} className={`h-full ${comp.color} rounded-full transition-all`}></div>
                    </div>
                  </div>
                );
              })}
          ...
          <div className="mt-6 pt-4 border-t border-slate-100 bg-slate-50 p-3 rounded-2xl">
            <p className="text-xs font-bold text-slate-700">Comparative Insight:</p>
            <p className="text-[11px] font-medium text-slate-500 mt-0.5">
              {revenueComparison.today >= revenueComparison.yesterday
                ? "🚀 Today's revenue is performing higher than yesterday."
                : "⚠️ Today's revenue is currently below yesterday's benchmark. Intensify admission follow-ups."}
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-6 bg-white border border-slate-200/90 rounded-3xl p-6 shadow-sm">
          <h2 className="text-base font-extrabold text-slate-800">Lead Conversion Funnel</h2>
          <p className="text-xs text-slate-400 font-medium mb-6">Stage-wise student journey, conversion rates & drop-offs</p>
          ...
          <div className="space-y-4">
            {[
              ...
              <div key={idx} className="bg-slate-50 border border-slate-200/60 rounded-2xl p-3.5 space-y-2">
                ...
                <div className="w-full h-2.5 bg-slate-200 rounded-full overflow-hidden">
                  <div style={{ width: `${Math.max(5, stg.pct)}%` }} className={`h-full ${stg.color} transition-all`}></div>
                </div>
        ...
        <div className="lg:col-span-6 bg-white border border-slate-200/90 rounded-3xl p-6 shadow-sm flex flex-col justify-between">
          <div>
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-base font-extrabold text-slate-800">Business Loss Analysis</h2>
            ...
            <div className="bg-rose-50/60 border border-rose-200/80 rounded-2xl p-4 mb-6">
              <p className="text-[10px] font-extrabold uppercase tracking-widest text-rose-600">Formula Calculation</p>
            ...
            <div className="grid grid-cols-3 gap-3 text-center">
              <div className="bg-slate-50 border border-slate-200/80 rounded-2xl p-3">
                ...
                <p className="text-sm font-extrabold text-slate-800 mt-1">{formatCurrency(businessLossAnalysis.potentialRevenue)}</p>
              </div>
              <div className="bg-slate-50 border border-slate-200/80 rounded-2xl p-3">
                <p className="text-[10px] font-bold text-slate-400 uppercase">Actual Revenue</p>
                <p className="text-sm font-extrabold text-emerald-600 mt-1">{formatCurrency(businessLossAnalysis.actualRevenue)}</p>
              </div>
              <div className="bg-slate-50 border border-slate-200/80 rounded-2xl p-3">
                <p className="text-[10px] font-bold text-slate-400 uppercase">Lost Opportunity</p>
                <p className="text-sm font-extrabold text-rose-600 mt-1">{businessLossAnalysis.lostOpportunityPct}%</p>
              </div>
            </div>
          </div>

          <div className="mt-6 pt-4 border-t border-slate-100">
            <p className="text-xs font-bold text-slate-700">Executive Impact Statement:</p>
      ...
      <div className="bg-white border border-slate-200/90 rounded-3xl p-6 shadow-sm">
        <h2 className="text-base font-extrabold text-slate-800 mb-1">Brand Performance Breakdown</h2>
        <p className="text-xs text-slate-400 font-medium mb-6">Cross-brand operational performance, collections & estimated loss</p>

        <div className="overflow-x-auto">
          <table className="w-full text-left text-xs font-medium">
            <thead>
              <tr className="bg-slate-50 border-y border-slate-200 text-slate-500 font-extrabold uppercase text-[10px] tracking-wider">
            ...
            <tbody className="divide-y divide-slate-100">
              {brandPerformance.map((brand, idx) => (
                <tr key={idx} className="hover:bg-slate-50/80 transition-colors">
                  <td className="py-3 px-4 font-bold text-slate-800">{brand.brandName}</td>
                  <td className="py-3 px-4 font-semibold text-slate-700">{brand.totalLeads}</td>
                  <td className="py-3 px-4 font-extrabold text-indigo-600">{brand.admissions}</td>
                  ...
                  <td className="py-3 px-4 font-bold text-rose-500">{formatCurrency(brand.estimatedBusinessLoss)}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <div className="bg-white border border-slate-200/90 rounded-3xl p-6 shadow-sm">
        <div className="flex items-center justify-between mb-4">
          <div>
            <h2 className="text-base font-extrabold text-slate-800">Sales Executive Performance Dashboard</h2>
            <p className="text-xs text-slate-400 font-medium">Ranked by admissions converted, conversion %, collections & follow-up status</p>
          </div>
            ...
            <tbody className="divide-y divide-slate-100">
              {counsellorPerformance.map((exec, idx) => (
                <tr key={idx} className={`hover:bg-slate-50/80 transition-colors ${exec.isTopPerformer ? "bg-emerald-50/30" : exec.isLowPerformer ? "bg-rose-50/30" : ""}`}>
                  ...
                  <td className="py-3 px-4 font-extrabold text-indigo-600">{exec.admissionsConverted}</td>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-7 bg-white border border-slate-200/90 rounded-3xl p-6 shadow-sm">
          <h2 className="text-base font-extrabold text-slate-800">Lead Source Analysis (Marketing ROI)</h2>
          <p className="text-xs text-slate-400 font-medium mb-6">Evaluating lead acquisition channels by revenue contribution</p>

          <div className="space-y-3">
            {leadSourceAnalysis.map((src, idx) => (
              <div key={idx} className="bg-slate-50 border border-slate-200/60 rounded-2xl p-3.5 flex items-center justify-between gap-4">
                <div>
                  <p className="text-xs font-extrabold text-slate-800">{src.source}</p>
                  <p className="text-[11px] text-slate-500 font-medium mt-0.5">
                    {src.leadsGenerated} Leads • {src.admissions} Admissions ({src.conversionRate}%)
                  </p>
                </div>
                <div className="text-right">
                  <p className="text-xs font-extrabold text-emerald-600">{formatCurrency(src.revenueContribution)}</p>
        ...
        <div className="lg:col-span-5 bg-white border border-slate-200/90 rounded-3xl p-6 shadow-sm">
          <h2 className="text-base font-extrabold text-slate-800">Collection Summary by Mode</h2>
          <p className="text-xs text-slate-400 font-medium mb-6">Payment mode distribution across active admissions</p>
            ...
            {collectionSummaryByMode.map((item, idx) => (
              <div key={idx} className="space-y-1">
                ...
                <div className="w-full h-2.5 bg-slate-100 rounded-full overflow-hidden">
                  <div style={{ width: `${Math.max(5, item.percentage)}%` }} className="h-full bg-emerald-500 rounded-full"></div>
                </div>
              </div>
            ))}
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-7 bg-white border border-slate-200/90 rounded-3xl p-6 shadow-sm">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-base font-extrabold text-slate-800">Pending Fee & Overdue EMI Summary</h2>
            <span className="text-xs font-bold text-rose-600 bg-rose-50 px-2.5 py-1 rounded-full border border-rose-100">
          ...
          <div className="grid grid-cols-2 gap-4 mb-6">
            <div className="bg-rose-50/70 border border-rose-200/80 rounded-2xl p-4">
              <p className="text-[10px] font-bold text-rose-600 uppercase">Overdue Balance</p>
              <p className="text-lg font-extrabold text-rose-600 mt-1">{formatCurrency(pendingFeeSummary.overdueAmount)}</p>
            </div>
            <div className="bg-amber-50/70 border border-amber-200/80 rounded-2xl p-4">
              <p className="text-[10px] font-bold text-amber-600 uppercase">Upcoming Installments</p>
              <p className="text-lg font-extrabold text-amber-600 mt-1">{formatCurrency(pendingFeeSummary.upcomingInstallmentsAmount)}</p>
            </div>
          </div>

          <p className="text-xs font-bold text-slate-700 mb-2">Priority Students Requiring Immediate Follow-up:</p>
          <div className="space-y-2">
            {pendingFeeSummary.studentsRequiringFollowup.map((st, idx) => (
              <div key={idx} className="bg-slate-50 border border-slate-200/60 rounded-xl p-3 flex items-center justify-between text-xs font-medium">
                <div>
                  <p className="font-bold text-slate-800">{st.fullName}</p>
                  <p className="text-[10px] text-slate-400">{st.course} • 📱 {st.mobileNumber}</p>
                </div>
                <div className="text-right">
                  <p className="font-extrabold text-rose-600">{formatCurrency(st.remainingBalance)}</p>
        ...
        <div className="lg:col-span-5 bg-white border border-slate-200/90 rounded-3xl p-6 shadow-sm">
          <h2 className="text-base font-extrabold text-slate-800 mb-1">Operational Alerts</h2>
          <p className="text-xs text-slate-400 font-medium mb-6">Automated notifications requiring management review</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-5 bg-white border border-slate-200/90 rounded-3xl p-6 shadow-sm">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-base font-extrabold text-slate-800">Tomorrow&apos;s Business Targets</h2>
            <span className="text-xs font-bold text-indigo-600 bg-indigo-50 px-2.5 py-1 rounded-full border border-indigo-100">
              Predictive Goals
            </span>
          </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            {[
              ...
              <div key={idx} className="bg-slate-50 border border-slate-200/70 rounded-2xl p-3.5">
                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">{tgt.label}</p>
                <p className={`text-sm font-extrabold mt-1 ${tgt.color}`}>{tgt.val}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="lg:col-span-7 bg-slate-900 text-white rounded-3xl p-6 shadow-xl flex flex-col justify-between">
          <div>
            <div className="flex items-center justify-between border-b border-slate-800 pb-3 mb-4">
              <div className="flex items-center gap-2">
                <span className="text-xl">🤖</span>
                <div>
                  <h2 className="text-base font-extrabold text-white">AI Business Insights & Executive Synthesis</h2>
                  <p className="text-[10px] text-indigo-300 font-medium">Automated strategic observation summary</p>
                </div>
              </div>
            ...
            <div className="space-y-3">
              <div>
                <p className="text-[11px] font-bold text-emerald-400 uppercase tracking-wider">Key Achievements</p>
              ...
              <div>
                <p className="text-[11px] font-bold text-amber-400 uppercase tracking-wider">Priority Actions for Tomorrow</p>
                <ul className="list-disc list-inside text-xs text-slate-300 space-y-0.5 mt-1">
          ...
          <div className="mt-6 pt-3 border-t border-slate-800 flex items-center justify-between text-[10px] text-slate-400 font-semibold">
            <span>CoachFlow Decision Support Engine v3.2</span>
