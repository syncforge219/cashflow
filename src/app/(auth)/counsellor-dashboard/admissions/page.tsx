...
export default function AdmissionHub() {
    ...
    const fetchAdmissions = async () => {
        ...
        try {
            ...
            if (json.success && Array.isArray(json.data)) {
                ...
                const uId = (user?.id || user?._id || "").toString().trim().toLowerCase();

                const isMatch = (dbVal: string) => {
                    const val = (dbVal || "").trim().toLowerCase();
                    if (!val || val === "staff" || val === "counsellor" || val === "unassigned" || val === "n/a") return true;
                    if (!uName && !uEmail) return false;

                    if (val === uName || val === uEmail || val === uId) return true;

                    const dbTokens = val.split(/\s+/).filter((t: string) => t.length > 1);
                    const uTokens = uName.split(/\s+/).filter((t: string) => t.length > 1);
                    if (uTokens.length > 0 && dbTokens.length > 0) {
                        if (uTokens.every((ut: string) => dbTokens.includes(ut)) || dbTokens.every((dt: string) => uTokens.includes(dt))) {
                            return true;
                        }
                    }
                ...
                const myAdmissions = json.data.filter((a: any) => isMatch(a.counsellor || a.assignedCrmAdvisor));
    ...
    const handleSearch = async () => {
        ...
        try {
            const res = await fetch(`/api/admissions/search?q=${encodeURIComponent(searchQuery)}`);
    ...
    return (
        <div className="flex h-screen bg-[#f8faff] text-slate-800 overflow-hidden font-sans">
            ...
            <div className="flex-1 flex flex-col min-w-0 overflow-y-auto relative">
                <div className="p-8 pb-32">
                    ...
                    <header className="mb-8">
                        <h1 className="text-[28px] font-extrabold tracking-tight text-slate-800 mb-2">Admission & Enrollment Hub</h1>
                    ...
                    <div className="bg-white border border-slate-200/80 rounded-2xl p-6 shadow-sm mb-8">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                            ...
                            <div className="md:col-span-2">
                                <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-2">Search by Name / Mobile / Parent Name / Email / ID</label>
                                    ...
                                    <div className="bg-rose-50 border border-rose-200 rounded-2xl p-6">
                                        <div className="flex items-center gap-3 mb-4">
                                            ...
                                            <div>
                                                ...
                                                <p className="text-xs font-semibold text-rose-600/80">This mobile number doesn't exist in our records.</p>
                                            </div>
                                        </div>
                                        <div className="flex items-center gap-3">
                                            <button className="bg-white border border-rose-200 hover:bg-rose-100 text-rose-700 text-xs font-bold px-4 py-2 rounded-xl transition-colors cursor-pointer">
                                                Create New Enquiry
                                            </button>
                                            <button className="bg-rose-600 hover:bg-rose-700 text-white text-xs font-bold px-4 py-2 rounded-xl transition-colors shadow-sm cursor-pointer">
                                            ...
                                            <div key={admItem._id || admItem.admissionId || index} className="bg-emerald-50 border border-emerald-200 rounded-2xl p-6">
                                                <div className="flex items-center gap-3 mb-6">
                                                    ...
                                                    <div>
                                                        <h3 className="text-sm font-bold text-emerald-900 flex items-center gap-2">
                                                            ...
                                                            <span className="bg-emerald-200 text-emerald-800 text-[9px] px-2 py-0.5 rounded-md uppercase tracking-wider font-mono">{admItem.admissionNumber || admItem.admissionId}</span>
                                                        </h3>
                                                        <p className="text-xs font-semibold text-emerald-700/80">This student is already enrolled in the system.</p>
                                                    </div>
                                                </div>
                                                ...
                                                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6 p-4 bg-white/60 rounded-xl border border-emerald-100">
                                                    ...
                                                    <div>
                                                        ...
                                                        <p className="text-xs font-bold text-emerald-950">{admItem.feeStatus || "Enrolled"} <span className="text-emerald-600 font-semibold">(₹{admItem.outstandingAmount ?? admItem.remainingBalance ?? 0} due)</span></p>
                                                    </div>
                                                </div>
                                                ...
                                                <div className="flex flex-wrap items-center gap-3">
                                                    <button className="bg-white border border-emerald-200 hover:bg-emerald-100 text-emerald-700 text-xs font-bold px-4 py-2 rounded-xl transition-colors cursor-pointer">
                                                        Open Admission Ledger
                                                    </button>
                                                    <button className="bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-bold px-4 py-2 rounded-xl transition-colors shadow-sm cursor-pointer">
                                                        Collect Fees
                                                    </button>
                                                    <button className="bg-white border border-emerald-200 hover:bg-emerald-100 text-emerald-700 text-xs font-bold px-4 py-2 rounded-xl transition-colors cursor-pointer">
                                ...
                                {enquiriesList.length > 0 && (
                                    <div className="space-y-4">
                                        ...
                                        {enquiriesList.map((lead: any, index: number) => {
                                            ...
                                            return (
                                                <div key={lead._id || index} className={c("bg-amber-50 border-amber-200", "bg-emerald-50 border-emerald-200") + " border rounded-2xl p-6"}>
                                                    <div className="flex items-start justify-between mb-6">
                                                        <div className="flex items-center gap-3">
                                                            <div className={c("bg-amber-100 text-amber-600", "bg-emerald-100 text-emerald-600") + " h-10 w-10 rounded-full flex items-center justify-center"}>
                                                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" className="w-5 h-5">
                                                            ...
                                                            <div>
                                                                ...
                                                                <p className={c("text-amber-700/80", "text-emerald-700/80") + " text-xs font-semibold"}>This prospect is already in the sales pipeline.</p>
                                                            </div>
                                                        </div>
                                                    </div>
                                                    ...
                                                    <div className={c("border-amber-100", "border-emerald-100") + " grid grid-cols-2 md:grid-cols-4 gap-4 mb-6 p-4 bg-white/60 rounded-xl border"}>
                                                        ...
                                                        <div>
                                                            <p className={c("text-amber-600/70", "text-emerald-600/70") + " text-[10px] font-bold uppercase"}>Last Follow-up</p>
                                                            <p className={c("text-amber-950", "text-emerald-950") + " text-xs font-bold"}>{lead.lastFollowUp || "None"}</p>
                                                        </div>
                                                        <div className="col-span-3">
                                                            <p className={c("text-amber-600/70", "text-emerald-600/70") + " text-[10px] font-bold uppercase"}>Next Follow-up</p>
                                                            <p className={c("text-amber-950", "text-emerald-950") + " text-xs font-bold"}>{lead.nextFollowUp || "None"}</p>
                                                        </div>
                                                    </div>
                                                    ...
                                                    {!isAdmitted && (
                                                        <div className="flex flex-wrap items-center gap-3">
                                                            ...
                                                            <button onClick={() => { setSelectedLead(lead); setOpenTaskModalOnLoad(false); setOpenDemoModalOnLoad(true); }} className="bg-purple-50 hover:bg-purple-100 border border-purple-200 text-purple-700 text-xs font-bold px-4 py-2 rounded-xl transition-colors cursor-pointer">
                                                                Schedule Demo
                                                            </button>
                                                            <button onClick={() => { setLeadForAdmission(lead); setIsAdmissionModalOpen(true); }} className="bg-emerald-500 hover:bg-emerald-600 text-white text-xs font-bold px-4 py-2 rounded-xl transition-colors shadow-sm shadow-emerald-500/20 ml-auto cursor-pointer">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
                        ...
                        <div className="bg-white border border-slate-200/80 rounded-2xl p-5 shadow-sm flex items-center justify-between">
                            <div>
                                <h3 className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-2">Today's Enrollments</h3>
                        ...
                        <div className="bg-white border border-slate-200/80 rounded-2xl p-5 shadow-sm flex items-center justify-between">
                            <div>
                                <h3 className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-2">Monthly Enrollment</h3>
                        ...
                        <div className="bg-white border border-slate-200/80 rounded-2xl p-5 shadow-sm flex items-center justify-between relative overflow-hidden">
                            <div className="w-full">
                                <h3 className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-2">Sales Conversion</h3>
                                ...
                                <div className="h-1 w-full bg-slate-100 rounded-full mt-2">
                                    <div className="h-1 bg-slate-200 rounded-full w-[0%]" />
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="bg-white border border-slate-200/80 rounded-2xl p-5 shadow-sm flex items-center justify-between">
                            <div>
                                <h3 className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-2">Enrollments Pool</h3>
                                <div className="text-3xl font-extrabold text-slate-800 mb-1">{admissions.length}</div>
                                <p className="text-[11px] font-semibold text-slate-400">Out of 0 leads</p>
                            </div>
                    ...
                    <div className="flex items-center gap-8 border-b border-slate-200 mb-6">
                        <button
                            ...
                            className={`flex items-center gap-2 pb-3 text-sm font-bold border-b-2 transition-colors ${activeTab === "Enrollment Ledgers" ? "border-indigo-600 text-indigo-700" : "border-transparent text-slate-500 hover:text-slate-700 hover:border-slate-300"
                                }`}
                        >
                    ...
                    <div className="bg-white border border-slate-200/80 rounded-2xl p-3 shadow-sm flex flex-col md:flex-row items-center gap-3 mb-6">
                        <div className="relative flex-1 w-full">
                            ...
                            <input
                                type="text"
                                placeholder="Search by student name, admission #, mobile, email, course..."
                        ...
                        <div className="flex items-center gap-3 w-full md:w-auto overflow-x-auto pb-2 md:pb-0 scrollbar-hide">
                            <select className="text-xs font-bold text-slate-700 bg-white border-none py-2 pr-8 focus:outline-none focus:ring-0 appearance-none cursor-pointer">
                            ...
                            <div className="flex items-center gap-2 border border-slate-200 rounded-lg px-3 py-1.5 cursor-pointer hover:bg-slate-50 transition-colors">
                                <span className="text-xs font-bold text-slate-700">dd-mm-yyyy</span>
                    ...
                    <div className="bg-white border border-slate-200/80 rounded-t-3xl min-h-[300px] flex flex-col shadow-sm overflow-hidden">
                        {isLoadingAdmissions ? (
                            ...
                            <div className="flex-1 flex flex-col items-center justify-center py-20">
                                <p className="text-sm font-bold text-slate-400">No admissions found.</p>
                            </div>
                        ) : (
                                        ...
                                        {admissions.map((adm, i) => (
                                            <tr key={i} className="hover:bg-slate-50/50 transition-colors cursor-pointer" onClick={() => setSelectedAdmissionDetail(adm)}>
                                                <td className="px-6 py-4">
                                                    <div className="flex items-center gap-3">
                                                        ...
                                                        <div>
                                                            ...
                                                            <p className="text-[10px] font-semibold text-slate-500">{adm.admissionId || "N/A"}</p>
                                                        </div>
                                                    </div>
                                                </td>
                                                <td className="px-6 py-4">
                                                ...
                                                <td className="px-6 py-4">
                                                    ...
                                                    <p className="text-[10px] font-semibold text-red-500">Bal: ₹{adm.remainingBalance}</p>
                                                </td>
                                                <td className="px-6 py-4">
            ...
            <LeadProfile
                ...
                onSuccess={() => { setSelectedLead(null); setOpenTaskModalOnLoad(false); setOpenDemoModalOnLoad(false); handleSearch(); }}
