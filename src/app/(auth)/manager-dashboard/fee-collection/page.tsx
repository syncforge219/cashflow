...
export default function FeeCollectionPage() {
    ...
    // Set default date and fetch recent admissions on load
    useEffect(() => {
    ...
    const fetchRecentAdmissions = async () => {
        try {
            ...
            const url = user?.brandScope ? `/api/admissions?brand=${encodeURIComponent(user.brandScope)}` : "/api/admissions";
    ...
    const handleSearchChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            ...
            const brandQuery = user?.brandScope ? `&brand=${encodeURIComponent(user.brandScope)}` : "";
            const res = await fetch(`/api/admissions?q=${encodeURIComponent(val)}${brandQuery}`);
            const data = await res.json();
            if (res.ok && data.success) {
    ...
    const handlePaymentSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (res.ok && resData.success) {
                ...
                const brandQuery = user?.brandScope ? `&brand=${encodeURIComponent(user.brandScope)}` : "";
                const freshRes = await fetch(`/api/admissions?q=${selectedStudent.admissionId}${brandQuery}`);
                                            ...
                                            >
                                                ...
                                                <div className="border-t border-slate-100 pt-3 flex justify-between items-center text-xs font-bold">
                                                    <div>
                                                        ...
                                                        <span className={(student.remainingBalance || 0) > 0 ? "text-rose-500" : "text-emerald-600"}>
                                                            ₹{(student.remainingBalance || 0).toLocaleString("en-IN", { minimumFractionDigits: 2 })}
                                                        </span>
                                                    </div>
